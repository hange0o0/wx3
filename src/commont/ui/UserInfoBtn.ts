...
class UserInfoBtn {
    ...
    public constructor(btn, fun:Function, parent:game.BaseUI_wx3, url:string) {
            ...
            this.okBtn.addEventListener(egret.TouchEvent.TOUCH_TAP,() => {
                ...
                window['wx'].authorize({
                    ...
                    fail:(res)=>{
                        if(this.force)
                        {
                            console.log(UM_wx3.loginSuccess,UM_wx3.gameid,UM_wx3.gameid2)
                            if(!UM_wx3.loginSuccess)
                            {
                                UM_wx3.getUserInfoZJ(()=>{
                                    this.openSetting();
                                },true)
                                return;
                            }
